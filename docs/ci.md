...
It must allow the GitHub Actions integration to push the generated registry
index; the administrator bypass is break-glass recovery, not the normal merge
path. Review requirements remain deferred as described under Phase B.
...
Actions ruleset bypass to write unmerged commits to `main`. Main-push runs are
serialized without cancelling an active run. GitHub coalesces pending runs to
the newest main revision; an active run also exits cleanly before publication
...
across newer source. A workflow-token push does not recursively start a run.
