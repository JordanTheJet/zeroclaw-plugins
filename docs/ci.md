...
It must allow only the repository's dedicated write-enabled registry deploy
key to bypass the gate for the generated index push. The private key lives only
in the `REGISTRY_DEPLOY_KEY` Actions secret; the administrator bypass is
...
otherwise a branch could change its own workflow guard and use the planned
deploy-key ruleset bypass to write unmerged commits to `main`. Main-push runs
are serialized without cancelling an active run. GitHub coalesces pending runs
to the newest main revision; an active run also exits cleanly before
publication if it has already been superseded.
...
across newer source. The generated commit includes `[skip ci]` so its
deploy-key-authenticated push does not recursively start another publication
run. Release API calls continue to use the run-scoped `GITHUB_TOKEN`; the
deploy key is used only by Git for the bounded `main` push.
...
- If the registry deploy key is exposed or unavailable, disable it before any
  retry, rotate both the repository deploy key and `REGISTRY_DEPLOY_KEY`
  secret, and keep the ruleset active throughout the rotation.
